...
def _is_micr_line(line):
    """Detect MICR band lines across all Nova OCR rendering styles."""
    if re.search(r'["\u201c\u201d\u2018\u2019*]\s*\d{5,8}\s*["\u201c\u201d\u2018\u2019*]', line):
        return True
    if re.search(r'\d{6,}\s*:{1,2}', line):
        return True
    if re.search(r'\d{6,}\s+\d{6,}', line):
        return True
    if re.search(r'[\u0900-\u097F].*\d{5,}', line):
        return True
    return False
...
def extract_micr_code(lines):
    """Extract the 9-digit MICR routing code from the MICR band.
    Indian cheque MICR band order: "CHQNO(6)" ROUTING(9): ACCOUNT" SERIAL
    Uses digit-group accumulation so OCR spaces inside the 9-digit field are tolerated.
    """
    for l in lines or []:
        if not _is_micr_line(l):
            continue
        ...
        for grp in digit_groups:
            if not found_chq:
                # cheque number is 5-8 digits — mark it found and skip it
                if 5 <= len(grp) <= 8:
                    found_chq = True
                continue
            # accumulate digits after cheque number until we reach 9
            buf += grp
            if len(buf) == 9:
                return buf
            if len(buf) > 9:
                return buf[:9]   # take first 9 if OCR merged fields
    return None
...
def parse_cheque_fields(lines, image_name=""):
    ...
    f['micr_code']     = extract_micr_code(lines)   # 9-digit MICR routing code
...
def merge_fields(model, fallback):
    ...
    out["micr_code"]              = fallback.get("micr_code")  # 9-digit routing code
    return out
...
def audit_fields(fields, raw_lines=None):
    ...
    # MICR routing code check — goes into fraud_reasons (tampering signal),
    # NOT into missing[] so it never appears as a NULL upsert
    micr = re.sub(r'\D', '', str(fields.get("micr_code") or ""))
    if len(micr) != 9:
        fraud_reasons.append("MICR code absent or unreadable on cheque")
...
try:
    ...
    cheque_number_map = {}   # for duplicate detection
    duplicate_cheques = []   # filled after loop
    ...
    for image_file in image_files:
        ...
        try:
            ...
            # duplicate cheque number tracking
    ...
    import re, uuid, datetime as _dt
    ...
    if failures:
        for line in failures:
            # Accept "file :: reason", "file: reason", or free text
            m = re.match(r"\s*(.*?)\s*::\s*(.*)\s*$", line) or re.match(r"\s*(.*?)\s*:\s*(.*)\s*$", line)
            img, reason = (None, line.strip())
            if m:
                img, reason = m.group(1).strip(), m.group(2).strip()
            rows.append({
                "cheque_number": img_to_cheque.get(img),
                "img_name": img,
                "is_fraud": None,
    ...
    schema = StructType([
        ...
        StructField("event_ts", StringType(), True),  # cast to timestamp below
    ...
    if TO_HIVE:
        # IMPORTANT:
        # 1) Create the external Hive table once using your chosen DB/TABLE and LOCATION.
        # 2) Set OUTPUT_PATH below to that LOCATION.
        # 3) After write, run MSCK/REFRESH outside or via spark.sql(...).
  ...
  # <-- set your path
        ...
        # If the table already exists and points to OUTPUT_PATH, you may also repair here:
