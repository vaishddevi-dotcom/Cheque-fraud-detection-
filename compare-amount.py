...
def parse_cheque_fields(lines: list, image_name: str) -> dict:
    ...
    amount_numbers         = None
    amount_words           = None
    amount_number_raw_line = None   # full SOIT line, used to check for /-
    bank_name       = None
    account_number  = None
    ifsc_code       = None
    ...
    for l in lines:
        ...
        if m1:
            ...
            if raw_val not in consumed_numeric_tokens:
                ...
                amount_number_raw_line = l.strip()   # preserve full line for /- check
                break
        ...
        for l in lines:
            ...
            for m2 in re.finditer(r'\b([0-9][\d,]{2,}(?:\.\d{1,2})?)\b', l):
                ...
                if d_count > 10 or d_count < 3 or (d_count <= 4 and int(re.sub(r"\D","",raw_val)) < 100):
                    continue
                amount_numbers         = raw_val
                amount_number_raw_line = l.strip()
                break
            if amount_numbers:
                break
    ...
    return {
        ...
        "amount_number_raw_line":  amount_number_raw_line,
...
def invoke_bedrock_ocr_and_extract(image_bytes: bytes, mime: str, model_id: str, region: str) -> dict:
    ...
    try:
        ...
        # ── Sanitize invalid \uXXXX escape sequences before parsing ──────────
        # Nova sometimes emits malformed unicode escapes (e.g. \uRs, \u20B9xx)
        # when OCRing the ₹ symbol, Devanagari script, or decorative fonts.
        # Replace any \u NOT followed by exactly 4 hex digits with a space.
        cleaned = re.sub(r'\\u(?![0-9A-Fa-f]{4})', ' ', cleaned)
        # Strip literal control characters that break JSON parsing
        cleaned = re.sub(r'[\x00-\x08\x0b\x0c\x0e-\x1f]', ' ', cleaned)

        try:
            return json.loads(cleaned)
        except json.JSONDecodeError:
            # JSON still broken after sanitization — build a best-effort dict
            # from raw text so the fallback parser can still extract fields.
            print(f"[WARN] Bedrock JSON parse failed after sanitization — "
                  f"using raw text lines only for this image.")
            raw_lines = [l.strip() for l in text_out.splitlines() if l.strip()]
            return {"lines": raw_lines, "fields": {}}

    except ClientError as e:
        raise RuntimeError(f"AWS ClientError invoking Bedrock model '{model_id}': {e}")
    except BotoCoreError as e:
        raise RuntimeError(f"Bedrock BotoCoreError: {e}")
...
# Indian English amount word system:
#   supports crore, lakh, thousand, hundred + all ones/teens/tens
_AMOUNT_ONES = {
...
_AMOUNT_MAGNITUDES = {
    'crore':10000000, 'lakh':100000, 'lac':100000, 'thousand':1000, 'hundred':100
}

def words_to_number(text: str):
    """
    Convert Indian English amount words to an integer.
    e.g. 'Thirty Thousand Only'                → 30000
         'Fifteen Thousand Three Hundred Only' → 15300
         'Two Hundred and Fifty Only'          → 250
    ...
    s = re.sub(r'\brupees?\b|\bonly\b|\band\b|[,\-]', ' ', s)
    s = re.sub(r'[^a-z\s]', ' ', s)          # remove digits/symbols
    ...
    for tok in tokens:
        if tok in _AMOUNT_ONES:
            current += _AMOUNT_ONES[tok]
        elif tok in _AMOUNT_MAGNITUDES:
            mag = _AMOUNT_MAGNITUDES[tok]
            if mag == 100:
                current = (current if current else 1) * 100
            else:
                total  += (current if current else 1) * mag
                current = 0
        # else: unknown token (paisa, decimal words, etc.) — ignored
    total += current
...
def audit_fields(fields: dict) -> list:
    ...
    # ── CHECK 1: '/-' must appear after Amount in Numbers ───────────────────
    # On a legitimate Indian cheque the SOIT box always ends with /-
    # e.g. "₹ 30,000/-"  or  "30000/-"
    # If the raw line doesn't contain /- the amount may have been tampered.
    raw_amt_line = fields.get("amount_number_raw_line") or ""
    amt_val      = fields.get("amount_numbers")
    if amt_val is not None and not _is_blank(raw_amt_line):
        if "/-" not in raw_amt_line:
            missing.append("Amount in Numbers missing /- suffix (possible tampering)")

    # ── CHECK 2: 'Only' must appear at end of Amount in Words ───────────────
    # Every legitimate Indian cheque ends the words field with "Only"
    # e.g. "Thirty Thousand Only"
    raw_words = (fields.get("amount_words") or "").strip()
    if raw_words:
        if not re.search(r'\bonly\b', raw_words, re.IGNORECASE):
            missing.append("Amount in words missing 'Only' suffix (possible tampering)")

    # ── CHECK 3: Amount in Numbers must equal Amount in Words ────────────────
    # Both checks above must pass (/-  and Only present) before we compare.
    # If either is missing we already flagged it above — skip mismatch check
    # to avoid double-flagging the same issue.
    slash_ok = "/-" in raw_amt_line if (amt_val is not None and raw_amt_line) else True
    only_ok  = bool(re.search(r'\bonly\b', raw_words, re.IGNORECASE)) if raw_words else True

    if slash_ok and only_ok and amt_val is not None and raw_words:
        num_from_words = words_to_number(raw_words)
        try:
            num_from_box = int(Decimal(str(amt_val).replace(",", "")).to_integral_value())
        except Exception:
            num_from_box = None

        if num_from_words is not None and num_from_box is not None:
            if num_from_words != num_from_box:
                missing.append(
                    f"Amount mismatch: SOIT box says {num_from_box} "
                    f"but words say {num_from_words} — cheque fraud"
                )

    return missing   # empty list = all fields present, no fraud indicators
...
def merge_fields(model_fields: dict, fallback_fields: dict) -> dict:
    ...
    out["amount_number_raw_line"]   = fallback_fields.get("amount_number_raw_line")
    return out
