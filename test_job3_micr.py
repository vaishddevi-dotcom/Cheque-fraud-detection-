...
def extract_cheque_number(lines, account_number=None):
    """Return (cheque_number, micr_present).
    micr_present=True  -> number came from the MICR band (bottom line).
    micr_present=False -> fell back to label/scan; MICR band was unreadable/absent.
    """
    acct = re.sub(r'\D', '', account_number or "")
    # Pass 1: MICR band — number extracted directly from MICR line
    for l in lines or []:
        if _is_micr_line(l):
            m = _MICR_RE.search(l)
            if m:
                ...
                return cand, True          # found in MICR band
    # Pass 2: explicit printed label e.g. "Cheque No: 098765"
    for l in lines or []:
        m = re.search(r'(?i)\b(?:che?que|chq|leaf)\s*(?:no|#)?\s*[:\-]?\s*([0-9 \-]{6,12})', l)
        if m:
            cand = re.sub(r'\D', '', m.group(1))
            if 6 <= len(cand) <= 10 and cand != acct:
                return cand, False         # found via label, not MICR
    ...
    for l in lines or []:
        ...
        for run in re.findall(r'(\d{6})', l):
            ...
            return run, False              # found via scan, not MICR
    return None, False
...
def parse_cheque_fields(lines, image_name=""):
    ...
    chq_num, micr_present = extract_cheque_number(lines, f.get('account_number'))
    f['cheque_number'] = chq_num
    f['micr_present']  = micr_present
...
def merge_fields(model, fallback):
    ...
    # micr_present comes only from fallback (pattern-based OCR extraction)
    out["micr_present"]           = fallback.get("micr_present", False)
    return out
...
def audit_fields(fields, raw_lines=None):
    ...
    # MICR band check — flagged as missing field if cheque number was NOT
    # extracted from the MICR line (fell back to label/scan or not found at all)
    if not fields.get("micr_present"):                        missing.append("MICR code")
...
try:
    ...
    cheque_number_map = {}   # cheque_number -> list of image filenames
    ...
    for image_file in image_files:
        ...
        try:
            ...
            # record cheque number for duplicate detection
    ...
    duplicate_cheques = [
        (chq_num, imgs)
        for chq_num, imgs in cheque_number_map.items()
        if len(imgs) > 1
    ]
    ...
    if duplicate_cheques:
        ...
        for chq_num, imgs in duplicate_cheques:
            print(f"  - Cheque No. {chq_num} found in {len(imgs)} images: {', '.join(imgs)}")
    if failures:
        print("\nFailure details:")
