...
def _is_micr_line(line):
    ...
    if re.search(r'\d{6,}\s*:{1,2}', line): return True
...
def extract_micr_code(lines):
    ...
    then accumulates subsequent digits until 9 are collected."""
    for l in lines or []:
        if not _is_micr_line(l): continue
        digit_groups = re.findall(r'\d+', l)
        found_chq = False; buf = ''
...
try:
    ...
    if frauds:
        print("\nFraud details (amount mismatch / missing suffix / words-numbers mismatch / MICR):")
    ...
    def _norm_type(reason: str) -> str:
        ...
        if "micr" in r and "absent" in r: return "micr_missing"
        if "upserted with null"  in r: return "upserted_with_null"
    ...
    if flagged:
        for line in flagged:
            ...
            if not m: continue
            img, fields_str = m.group(1).strip(), m.group(2).strip()
            for fld in [x.strip() for x in fields_str.split(",") if x.strip()]:
                rows.append({
                    "cheque_number": img_to_cheque.get(img),
                    "img_name":      img,
                    "is_fraud":      None,
                    "fraud_reason":  f"Upserted with NULL in: {fld}",
                    "fraud_type":    "upserted_with_null",
                    "event_ts":      event_iso,
                    "proc_date":     proc_date,
                    "run_id":        run_id
                })
    ...
    TO_HIVE = False
    if TO_HIVE:
        OUTPUT_PATH = "s3a://hexaware-buk-75dbbff6/data/warehouse/tablespace/external/hive/cheque_analytics/demo_tb"
        (df
          .write
          .mode("append")
          .format("parquet")
          ...
          .save(OUTPUT_PATH))
        print(f"\nData written to {OUTPUT_PATH}. Please run MSCK REPAIR TABLE or REFRESH TABLE on your Hive table to load new partitions.")
        # spark.sql("MSCK REPAIR TABLE cheque_analytics.cheque_fraud_facts")
