...
PHOENIX_PASS     =  "Vaish@cloudera1"
SUPPORTED_EXT    = {".png", ".jpg", ".jpeg", ".tif", ".tiff", ".bmp", ".webp", ".gif"}
OUTPUT_PATH = "s3a://hexaware-buk-75dbbff6/data/warehouse/tablespace/external/hive/cheque_analytics/demo_tb" # Temporary output path for Spark write (will be moved to final HIVE_METADATA_PATH)
# HIVE METADATA CONFIG
HIVE_DB                  = "cheque_analytics"
HIVE_METADATA_TABLE      = "cheque_metadata"
HIVE_METADATA_PATH       = (
    "s3a://hexaware-buk-75dbbff6/data/warehouse/tablespace/external/hive/"
    "cheque_analytics/cheque_metadata"
)
# COD CONFIG
PHOENIX_VERIFY   = False
RECREATE_TABLE_COD   = False   # DROP+recreate Cheque_metadata each run; set False after first 
to_COD = True
#Hive metadata configx
RECREATE_HIVE_METADATA   = False
TO_HIVE_METADATA         = True
#Hive Finaltable config
TO_HIVE = True
...
# ---------------- CITY NAME EXTRACTOR (NEW) ----------------

_COMMON_CITIES = [
    "MUMBAI","DELHI","NEW DELHI","BENGALURU","BANGALORE","CHENNAI","HYDERABAD",
    "PUNE","AHMEDABAD","JAIPUR","KOLKATA","NOIDA","GURGAON","GURUGRAM",
    "FARIDABAD","CHANDIGARH","LUCKNOW","KANPUR","INDORE","BHOPAL",
    "NAGPUR","SURAT","VADODARA","RAJKOT","THANE","NAVI MUMBAI"
]
...
def normalize_city(raw):
    ...
    # ----- ✅ OCR CORRECTION RULES (MINIMAL & SAFE) -----
    # Treat common OCR errors as NEW DELHI
    if re.search(r'\bNEW\s+DE[FK]I\s*H?I?\b', s):
        return "NEW DELHI"

    # Handle space-less or noisy variants
    if re.search(r'\bNEW\s*DEL\s*HI\b', s):
        return "NEW DELHI"
...
def extract_city_from_lines(lines):
    """
    Priority:
    1. Explicit city names in address blocks
    2. Pin‑code paired city
    3. Known common cities
    """

    # Pass 1: address lines with known city keywords
    for l in lines or []:
        if not _is_address_line(l):
            continue
        for city in _COMMON_CITIES:
            if re.search(rf'\b{re.escape(city)}\b', l.upper()):
                return city

    # Pass 2: city near PIN code
    for l in lines or []:
        if re.search(r'\b\d{6}\b', l):   # PIN detected
            txt = normalize_city(l)
            if not txt:
                continue
            for city in _COMMON_CITIES:
                if city in txt:
    ...
    # Pass 3: standalone ALL‑CAPS city line
    for l in lines or []:
        s = normalize_city(l)
        if s in _COMMON_CITIES:
            return s
...
def _is_micr_line(line):
    """ANY non-digit/non-space char wrapping a digit group = MICR E13B delimiter.
    Covers: straight/curly quotes, Bengali chars (ই ৵), superscripts, asterisks.
    """
    if re.search(r'[^\d\s]\s*\d{5,8}\s*[^\d\s]', line): return True
    if re.search(r'\d{5,}\s*:{1,2}', line): return True   # relaxed from 6+ to 5+
    if re.search(r'\d{6,}\s+\d{6,}', line): return True
    # Also catch lines where a quoted digit group appears (even with only one side quoted)
    if re.search(r'["\u201C\u201D]\s*\d{5,8}', line): return True
    if re.search(r'\d{5,8}\s*["\u201C\u201D]', line): return True
    return False
...
# MICR E13B band has three fields: [cheque_no] [routing_code] [account_no]
# OCR may quote each field differently:
#   Pattern A (both sides): "482055" — reliable
#   Pattern B (leading only): "482055 ... — leading quote, no trailing on this group
#   Pattern C (trailing only): 482055" — trailing quote only (common for last-position fields)
# Strategy: collect ALL quoted candidates, prefer 6-digit (Indian cheque numbers are always 6 digits)
_MICR_RE = re.compile(
    r'["\u201C\u201D][\u0900-\u097F\s*]*(\d{5,8})[\u0900-\u097F\s*]*["\u201C\u201D*]'
)
# Trailing-quote-only: digit group immediately before a closing quote
_MICR_RE_TRAIL = re.compile(
    r'(?<!["\u201C\u201D\d])(\d{5,8})[\u0900-\u097F\s*]*["\u201C\u201D]'
)

def extract_cheque_number(lines, account_number=None):
    acct = re.sub(r'\D', '', account_number or "")
    # Pass 1: MICR band — collect ALL quoted digit candidates (both-sided + trailing-only quotes)
    for l in lines or []:
        if not _is_micr_line(l): continue
        cands = [m.group(1) for m in _MICR_RE.finditer(l)]
        cands += [m.group(1) for m in _MICR_RE_TRAIL.finditer(l)]
        # Deduplicate while preserving order
        seen = set(); unique = []
        for c in cands:
            if c not in seen: seen.add(c); unique.append(c)
        # Filter out account number matches
        unique = [c for c in unique
                  if not (acct and (c == acct or c in acct or acct in c))
                  and not (acct and len(c) >= 9)]
        if not unique: continue
        # Indian cheque numbers are always exactly 6 digits — prefer those
        six_digit = [c for c in unique if len(c) == 6]
        if six_digit: return six_digit[0]
        return unique[0]  # fallback: first remaining candidate
...
def extract_micr_code(lines):
    """Extract 9-digit MICR routing code. Skips first 5-8 digit group (cheque number),
    then accumulates subsequent PURE digit groups until 9 digits are collected.
    Stops accumulation if a non-pure-digit group (e.g. '0505E8') is encountered mid-stream."""
    for l in lines or []:
        if not _is_micr_line(l): continue
        # Split on non-alphanumeric boundaries; keep only pure-digit tokens
        all_tokens = re.findall(r'[A-Z0-9]+', l.upper())
        digit_groups = [t for t in all_tokens if t.isdigit()]
        found_chq = False; buf = ''
        for grp in digit_groups:
            if not found_chq:
                if 5 <= len(grp) <= 8: found_chq = True
                continue
            buf += grp
            if len(buf) == 9: return buf
            if len(buf) > 9: return buf[:9]
    return None
...
def parse_cheque_fields(lines, image_name=""):
    f = {k: None for k in ['cheque_number','cheque_date','payee_name',
         'amount_numbers','amount_words','bank_name','city','account_number',
         'ifsc_code','image_name','image_uploaded_time','amount_number_raw_line']}
    f['city']      = extract_city_from_lines(lines)
    ...
    f['micr_code']     = extract_micr_code(lines)
...
def invoke_bedrock(image_bytes, mime, model_id):
    ...
    prompt = (
        ...
        "- city: city name from address or branch location (e.g. PUNE, MUMBAI, BANGALORE, DELHI  )\n"
...
def ensure_table_exists(conn):
    ...
    cur = conn.cursor()
    try:
        if RECREATE_TABLE_COD:
            try: cur.execute('DROP TABLE IF EXISTS "Cheque_metadata"')
            except Exception: pass
        try:
            cur.execute(ddl)
            print("[COD] Cheque_metadata table ready.")
        except Exception as e:
            if "already exists" not in str(e).lower(): raise
            print("[COD] Cheque_metadata already exists — skipping CREATE.")
    finally:
        try: cur.close()
        except Exception: pass
...
def merge_fields(model, fallback):
    ...
    for k in ['cheque_number','cheque_date','payee_name','amount_numbers',
              'amount_words','bank_name','city','account_number','ifsc_code']:
        mv = (model or {}).get(k)
    ...
    # Indian cheque numbers are always exactly 6 digits.
    # If the model returned something other than 6 pure digits, trust the regex fallback instead.
    chq_digits = re.sub(r'\D', '', str(out.get("cheque_number") or ""))
    if len(chq_digits) != 6:
        fb_chq = re.sub(r'\D', '', str(fallback.get("cheque_number") or ""))
        if len(fb_chq) == 6:
            out["cheque_number"] = fb_chq
        # else keep whatever we have (both are wrong; don't discard)
    ...
    out["micr_code"]              = fallback.get("micr_code")
    return out
...
def words_to_number(text):
    ...
    t = re.sub(r'[^\x00-\x7F]+', ' ', t)   # strip ALL non-ASCII (Hindi/Bengali/etc)
    t = re.sub(r'[^a-zA-Z0-9\s]', ' ', t).lower().strip()
    t = re.sub(r'\s+', ' ', t).strip()       # collapse whitespace after strip
...
def validate_amount_format(fields, raw_lines=None):
    """Fraud checks: /- suffix, Only in words, words==numbers, MICR present."""
    reasons   = []
    ...
    # CHECK 1: /- suffix — fix: ₹ and /- may be on adjacent lines (OCR split)
    ...
    # CHECK 2: 'Only' in amount words
    # Fix: skip when amount_words has no English — purely non-English script cannot
    # be checked for the word 'Only' without false positives
    if amt_num is not None or amt_words:
        amt_words_has_english = bool(re.search(r'[a-zA-Z]', amt_words))
        if amt_words_has_english:
            has_only = False
            for l in raw_lines:
                if re.search(r'(?i)\bonly\b', re.sub(r'[^\x00-\x7F]+', ' ', l)):
                    has_only = True; break
            if not has_only:
                has_only = bool(re.search(r'(?i)\bonly\b',
                                re.sub(r'[^\x00-\x7F]+', ' ', amt_words)))
            if not has_only:
                reasons.append("Amount in words missing 'Only'")

    # CHECK 3: words == numbers
    ...
    # CHECK 4: MICR routing code (9-digit field in MICR band)
    # Fraud signal — goes into fraud_reasons, NOT missing[] (no NULL upsert noise)
    micr = re.sub(r'\D', '', str(fields.get("micr_code") or ""))
    if len(micr) != 9:
        reasons.append("MICR code absent or unreadable on cheque")
...
def ensure_hive_metadata_table():
    spark.sql(f"CREATE DATABASE IF NOT EXISTS {HIVE_DB}")

    if RECREATE_HIVE_METADATA:
        spark.sql(f"DROP TABLE IF EXISTS {HIVE_DB}.{HIVE_METADATA_TABLE}")

    spark.sql(f"""
        CREATE EXTERNAL TABLE IF NOT EXISTS {HIVE_DB}.{HIVE_METADATA_TABLE} (
            cheque_number          STRING,
            account_number         STRING,
            cheque_date            DATE,
            payee_name             STRING,
            amount_in_numbers      DECIMAL(18,2),
            amount_in_words        STRING,
            bank_name              STRING,
            city                   STRING,
            ifsc_code              STRING,
            image_name             STRING,
            image_uploaded_time    TIMESTAMP
        )
        PARTITIONED BY (proc_date STRING)
        STORED AS PARQUET
        LOCATION '{HIVE_METADATA_PATH}'
    """)

def _to_spark_decimal(v):
    """
    Convert value to decimal.Decimal for Spark DecimalType columns.
...
def build_hive_metadata_df(all_fields_by_image):
    ...
    schema = StructType([
        ...
        StructField("cheque_date", DateType(), True),
        StructField("payee_name", StringType(), True),
        StructField("amount_in_numbers", DecimalType(18,2), True),
        StructField("amount_in_words", StringType(), True),
        StructField("bank_name", StringType(), True),
        StructField("city", StringType(), True),
        StructField("ifsc_code", StringType(), True),
        StructField("image_name", StringType(), True),
        StructField("image_uploaded_time", TimestampType(), True),
        StructField("proc_date", StringType(), True)
    ])

    rows = []
    proc_date = _dt.datetime.now().strftime("%Y-%m-%d")

    for fields in all_fields_by_image.values():
        rows.append({
            "cheque_number":       fields.get("cheque_number"),
            "account_number":      fields.get("account_number"),
            "cheque_date":         _to_date(fields.get("cheque_date")),
            "payee_name":          fields.get("payee_name"),
            "amount_in_numbers":   _to_spark_decimal(fields.get("amount_numbers")),
            "amount_in_words":     fields.get("amount_words"),
            "bank_name":           fields.get("bank_name"),
            "city":                fields.get("city"),
            "ifsc_code":           fields.get("ifsc_code"),
            "image_name":          fields.get("image_name"),
            "image_uploaded_time": _to_ts(fields.get("image_uploaded_time")),
            "proc_date":           proc_date
        })
...
try:
    ...
    frauds = []; failures = []; seen_pks = set()
    cheque_number_map = {}   # cheque_number -> [image filenames]
    duplicate_cheques = []   # built after loop
    all_images_info   = {}   # img_basename -> {"cheque_number": ..., "is_fraud": False}
    all_metadata_fields = {} # img_basename -> {all extracted fields}
    ...
    for image_file in image_files:
        ...
        try:
            ...
            all_metadata_fields[os.path.basename(image_file)] = dict(fields)            #  Track every successfully parsed image

            all_images_info[os.path.basename(image_file)] = {
                "cheque_number": str(fields.get("cheque_number") or "").strip() or None
            }
            ...
            log = dict(fields)
            log["amount_numbers"] = _to_decimal_str(fields.get("amount_numbers"))
            log["micr_code"]      = fields.get("micr_code") or "NOT FOUND"
            ...
            # Bank name vs IFSC prefix validation
            bank_name = fields.get("bank_name")
            ifsc_code = fields.get("ifsc_code")
            if bank_name and ifsc_code:
                exp_prefix = _expected_ifsc_prefix_for_bank(bank_name)
                if exp_prefix and not ifsc_code.upper().startswith(exp_prefix):
            ...
            if missing:
                ...
                fraud_reasons.extend([f"Missing field: {fld}" for fld in missing])
                fraud_reasons.append("upserted_with_null")   

            chq_num = str(fields.get("cheque_number") or "").strip()
            if chq_num and chq_num.upper() not in ("", "NONE"):
                cheque_number_map.setdefault(chq_num, []).append(os.path.basename(image_file))
            ...
            pk = (
                str(fields.get("cheque_number", "")).strip(),
            ...
            # ---- IN‑PLACE COD UPSERT (FLAG CONTROLLED) ----
            if to_COD:
                try:
                    cod_conn = phoenix_connect()
                    ensure_table_exists(cod_conn) # Ensure table exists before first upsert
                    upsert_into_cod(cod_conn, fields)
                    upserts_ok += 1
                except Exception as e:
                    msg = f"{os.path.basename(image_file)} COD upsert failed: {repr(e)}"
                ...
                print("[DRY-RUN] COD write skipped (to_COD=False)")
    ...
    print(f"[DEBUG] Hive metadata rows captured: {len(all_metadata_fields)}")

    # build duplicate cheque list
    duplicate_cheques = [
        (num, imgs) for num, imgs in cheque_number_map.items() if len(imgs) > 1
    ]
    ...
    print(f"Duplicate cheques: {len(duplicate_cheques)}")
    print(f"Failures         : {len(failures)}")
    if frauds:
        print("\nFraud details (missing fields / amount mismatch / missing suffix / MICR / signature / stale):")
        for f in frauds: print(f"  - {f}")
    if duplicate_cheques:
        print("\nDuplicate cheque numbers detected:")
        for num, imgs in duplicate_cheques:
            print(f"  - Cheque No. {num} found in {len(imgs)} images: {', '.join(imgs)}")
    if failures:
        print("\nFailure details:")
        for f in failures: print(f"  - {f}")
    print("#"*70)

    # ========================= NEW: BUILD & SHOW TABLE =========================
    # Creates a Spark DataFrame from flagged/frauds/duplicates/failures and prints it.
    import uuid, datetime as _dt
    from pyspark.sql import functions as F
    from pyspark.sql.types import StructType, StructField, StringType, BooleanType, DateType, DecimalType,TimestampType
    


    proc_date = _dt.datetime.now().strftime("%Y-%m-%d")
    event_iso = _dt.datetime.now().isoformat()
    run_id    = str(uuid.uuid4())

    def _norm_type(reason: str) -> str:
        r = (reason or "").lower()
        if "signature missing"   in r: return "signature_missing"
        if "missing '/-'" in r or "suffix" in r: return "amount_suffix_missing"
        if "amount mismatch"     in r: return "amount_mismatch"
        if "missing 'only'"      in r: return "amount_words_only_missing"
        if "stale cheque"        in r: return "stale_cheque"
        if "bank/ifsc mismatch"  in r: return "bank_ifsc_mismatch"
        if "micr" in r and ("absent" in r or "unreadable" in r): return "micr_missing"
        if "missing field"       in r: return "missing_field"
        if "duplicate"           in r: return "duplicate_cheque_number"
        if "failure"             in r: return "failure"
        return "missing_field"

    rows = []
    img_to_cheque = {}

    # Duplicates first (to build img->cheque map)
    if duplicate_cheques:
        for num, imgs in duplicate_cheques:
            for img in imgs:
                img = img.strip()
                rows.append({
                    "cheque_number": str(num).strip(),
                    "img_name":      img,
                    "is_fraud":      True,
                    "fraud_reason":  "Duplicate cheque number across multiple images",
                    "fraud_type":    "duplicate_cheque_number",
                    "event_ts":      event_iso,
                    "proc_date":     proc_date,
                    "run_id":        run_id
                })
                img_to_cheque[img] = str(num).strip()

    # Frauds (now includes missing-field entries merged in from the loop)
    if frauds:
        for line in frauds:
            m = re.match(r"\s*(.*?)\s*::\s*(.*)\s*$", line)
            if not m: continue
            img, reasons_str = m.group(1).strip(), m.group(2).strip()
            for r in [x.strip() for x in reasons_str.split("|") if x.strip()]:
                rows.append({
                    "cheque_number": img_to_cheque.get(img),
                    "img_name":      img,
                    "is_fraud":      True,
                    "fraud_reason":  r,
                    "fraud_type":    _norm_type(r),
                    "event_ts":      event_iso,
                    "proc_date":     proc_date,
                    "run_id":        run_id
                })

    # Failures (operational)# Failures are operational issues and are intentionally NOT persisted to Hive to avoid noise in the fraud analytics. They are printed in the summary but not included in the Hive table.
    # if failures:
    #     for line in failures:
    #         m = (re.match(r"\s*(.*?)\s*::\s*(.*)\s*$", line) or
    #              re.match(r"\s*(.*?)\s*:\s*(.*)\s*$",  line))
    #         img, reason = (None, line.strip())
    #         if m: img, reason = m.group(1).strip(), m.group(2).strip()
    #         rows.append({
    #             "cheque_number": img_to_cheque.get(img),
    #             "img_name":      img,
    #             "is_fraud":      None,
    #             "fraud_reason":  f"Failure: {reason}",
    #             "fraud_type":    "failure",
    #             "event_ts":      event_iso,
    #             "proc_date":     proc_date,
    #             "run_id":        run_id
    ...
    # Clean cheques — images that processed successfully with no fraud and no missing fields
    imgs_with_issues = set()
    for line in frauds:
        m = re.match(r"\s*(.*?)\s*::\s*", line)
        if m: imgs_with_issues.add(m.group(1).strip())
    for line in failures:
        m = (re.match(r"\s*(.*?)\s*::\s*", line) or re.match(r"\s*(.*?)\s*:\s*", line))
        if m: imgs_with_issues.add(m.group(1).strip())

    for img, info in all_images_info.items():
        if img not in imgs_with_issues:
            rows.append({
                "cheque_number": info.get("cheque_number") or img_to_cheque.get(img),
                "img_name":      img,
                "is_fraud":      False,
                "fraud_reason":  None,
                "fraud_type":    "clean",
    ...
    print("\n=== Parsed Cheque Fraud Table (verify below) ===")
    df.orderBy(F.col("img_name").asc_nulls_last(), F.col("fraud_type")).show(200, truncate=False)

    # print("\n=== Quick counts by fraud_type (sanity check) ===")
    # df.groupBy("fraud_type").count().orderBy(F.desc("count")).show(truncate=False)

    # print("\n=== Minimal view (your requested columns only) ===")
    # df.select("cheque_number", "img_name", "is_fraud", "fraud_reason", "event_ts") \
    #   .orderBy(F.col("img_name").asc_nulls_last(), F.col("fraud_reason")).show(200, truncate=False)

    # ---------- READY SWITCH: Write to Hive location (keep False for now) ----------
    if TO_HIVE:
        (df
          .write
          .mode("append")
          .format("parquet")
          .partitionBy("proc_date")
          .save(OUTPUT_PATH))
        print(f"\nData written to {OUTPUT_PATH}.")

        #Running MSCK REPAIR TABLE or REFRESH TABLE on your Hive table to load new partitions.
        try:
            spark.sql("MSCK REPAIR TABLE cheque_analytics.demo_tb")
            spark.sql("REFRESH cheque_analytics.demo_tb")
            print("Hive metadata refreshed successfully.")
        except Exception as e:
    ...
    if TO_HIVE_METADATA:
        ensure_hive_metadata_table()

        hive_df = build_hive_metadata_df(all_metadata_fields)
        ...
        (
            ...
            .format("parquet")
            .partitionBy("proc_date")
            .save(HIVE_METADATA_PATH)
        )

        try:
            spark.sql(f"MSCK REPAIR TABLE {HIVE_DB}.{HIVE_METADATA_TABLE}")
            spark.sql(f"REFRESH TABLE {HIVE_DB}.{HIVE_METADATA_TABLE}")
            print("[HIVE] Metadata table updated successfully.")
        except Exception as e:
            print(f"[HIVE] Metadata refresh failed: {e}")
    else:
        print("[DRY-RUN] Hive metadata write skipped (TO_HIVE_METADATA=False)")

                    
    # ======================= END NEW: BUILD & SHOW TABLE =======================
