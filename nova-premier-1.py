    ...
    user_content = [
        ...
        },
        {
            "text": (
                ...
                "   - cheque_number  : 6-10 digit serial number printed on the cheque leaf\n"
...
def upsert_into_cod(conn, fields: dict):
    ...
    values = (
        ...
        _to_sql_decimal(fields.get("amount_numbers")),
...
def readback_from_cod(conn, cheque_number: str):
    ...
    try:
        ...
        if not row:
            ...
            return
        cols = ["Cheque number","Date on Cheque","Payee name","Amount in Numbers",
                "Amount in words","bank name","Account number","IFSC code",
                "image name","image uploaded time"]
        print("\n[COD] Read-back row:")
        for c, v in zip(cols, row):
            print(f"  {c}: {fmt_money(v) if c == 'Amount in Numbers' else v}")
        print()
    finally:
        if cur:
            try: cur.close()
...
def detect_fraud(fields: dict):
    ...
    if not (6 <= len(acct_digits) <= 20):
        ...
        fields["account_number"] = acct_digits
...
try:
    ...
    for image_file in image_files:
        ...
        try:
            ...
            if is_fraud:
                print(f"\n[FRAUD] {fields.get('image_name', os.path.basename(image_file))} "
                      f"is fraud – missing/null: {', '.join(missing_cols)}\n")
